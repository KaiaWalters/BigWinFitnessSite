...
const HamburgerMenu = ({ menuOpen, setMenuOpen }) => {
  return (
    <button
      ...
      className="relative inline-flex items-center justify-center rounded-md p-2 text-gray-800 hover:bg-gray-700 hover:text-white focus:ring-2 focus:ring-white focus:outline-hidden focus:ring-inset"
...
const MobileMenu = ({ menuOpen }) => {
  return (
    <div className={`sm:hidden transition-all duration-300 ease-in-out ${menuOpen ? 'block' : 'hidden'}`} id="mobile-menu">
      <div className="space-y-1 px-2 pt-2 pb-3">
        ...
        <Link to="/events" className="block rounded-md px-3 py-2 text-base font-medium text-gray-700 hover:bg-gray-700 hover:text-white">Events</Link>
      </div>
    </div>
  );
};
